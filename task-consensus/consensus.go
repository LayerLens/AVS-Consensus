...
// Type T is a new task request for operators to create a new score ofr a model.
...
type scoreFSM[T any, K any, S any] struct {
...
func NewConcensusFSM[T any, K any, S any](keyPair *bls.KeyPair, pk *ecdsa.PrivateKey, blsAggregationService blsagg.BlsAggregationService, ethClient eth.Client, logger logging.Logger) *scoreFSM[T, K, S] {
	return &scoreFSM[T, K, S]{
...
func (p *scoreFSM[T, K, S]) ConfigureRaftBindings(rpcUrl string, httpUrl string, fileStorageDirectory string, operatorId [32]byte) {
	p.RaftBind = rpcUrl
...
func (p *scoreFSM[T, K, S]) ConfigureTaskCallbacks(onTaskRequestFn onTaskRequest[T, K], onTaskResponseFn onSubmitTaskToLeader[T, K, S]) {
...
func (p *scoreFSM[T, K, S]) Initialize(enableSingle bool, localId string) error {
...
func (p *scoreFSM[T, K, S]) InitializeHttpServer(addr string, onLeaderProcessBlsSignedResponse onLeaderProcessBlsSignedResponse[K], isValidOperator isValidOperator, fetchOperatorUrl fetchOperatorUrl) error {
	h := &Service[K]{
		addr:                             addr,
		scoreFSM:                         p,
...
func (p *scoreFSM[T, K, S]) JoinExistingNetwork(joinAddr, raftAddr, nodeID string, latestBlock uint64) error {
...
func (p *scoreFSM[T, K, S]) setOperatorId(id sdktypes.OperatorId) {
	p.operatorId = id
}

func (p *scoreFSM[T, K, S]) setOnTaskRequestFn(fn onTaskRequest[T, K]) {
	p.onTaskRequestFn = fn
}

func (p *scoreFSM[T, K, S]) setOnTaskResponseFn(fn onSubmitTaskToLeader[T, K, S]) {
	p.onTaskResponseFn = fn
}

// Join joins a node, identified by nodeID and located at addr, to this store.
// The node must be ready to respond to Raft communications at that address.
func (p *scoreFSM[T, K, S]) Join(nodeID, addr string) error {
...
func (p *scoreFSM[T, K, S]) IsLeader() (bool, string) {
	leaderURL, _ := p.raft.LeaderWithID()
	return string(leaderURL) == p.RaftBind, string(leaderURL)
}

func (p *scoreFSM[T, K, S]) TriggerElection() {
	p.raft.LeadershipTransfer()
}

func (p *scoreFSM[T, K, S]) SubmitTaskToLeader(request T, responses []K) error {
...
func (p *scoreFSM[T, K, S]) LeaderSendTaskRequestToFollowers(cmd []byte) error {
...
func (f *scoreFSM[T, K, S]) Apply(l *raft.Log) interface{} {
...
func (f *scoreFSM[T, K, S]) Snapshot() (raft.FSMSnapshot, error) {
	return &fsmSnapshot{}, nil
}

// Restore stores the key-value store to a previous state.
func (f *scoreFSM[T, K, S]) Restore(rc io.ReadCloser) error {
	return nil
}
