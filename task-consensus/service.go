...
func NewService[K any](addr string, scoreFSM IscoreFSM, blsAggregationService blsagg.BlsAggregationService, ethClient eth.Client) *Service[K] {
	return &Service[K]{
		addr:                  addr,
		scoreFSM:              scoreFSM,
...
func (s *Service[K]) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path == "/submitAvsTask" {
		s.handlescoreUpdateTaskSubmittion(w, r)
...
func (s *Service[K]) handleJoin(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.scoreFSM.Join(nodeID, validOperatorUrls.RpcUrl); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func (s *Service[K]) handlescoreUpdateTaskSubmittion(w http.ResponseWriter, r *http.Request) { // handle task submission with 2 generic + 1 callback
	...
	// Submit each score feed source seperatly
